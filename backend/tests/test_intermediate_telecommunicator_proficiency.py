...
def test_basic_certificate_is_required(app):
    with app.app_context():
...
def test_two_year_service_requirement(app):
    with app.app_context():
        agency, officer = make_telecommunicator(
...
def test_120_training_hours_required(app):
    with app.app_context():
        agency, officer = make_telecommunicator()

        # Required courses produce 40 hours.
        add_required_courses(
            agency,
            officer,
        )

        # 70 more produces only 110 total.
        add_general_hours(
            agency,
            officer,
            70,
        ...
        assert result["training_hours"] == 110
        assert result["status"] == "NOT_ELIGIBLE"
        assert (
...
def test_training_hours_include_all_tcole_training(app):
    with app.app_context():
        agency, officer = make_telecommunicator()
        ...
        assert result["training_hours"] == 120
...
def test_tdd_tty_must_be_within_six_months(app):
    with app.app_context():
        agency, officer = make_telecommunicator()
...
def test_all_requirements_establish_eligibility(app):
    with app.app_context():
        ...
        assert result["status"] == "ELIGIBLE"
        assert result["service_years"] >= 2
        assert result["training_hours"] == 120
        assert result["missing_requirements"] == []


def test_intermediate_certificate_reports_awarded(app):
    with app.app_context():
        agency, officer = make_telecommunicator()
