    ...
    if advanced:
        db.session.add(
            OfficerAward(
                ...
                officer_id=officer.id,
                award_type="Certificate",
                award_name="Advanced Telecommunicator",
...
def test_advanced_certificate_required(app):
    with app.app_context():
        agency, officer = make_telecommunicator(
            advanced=False,
        )
...
def test_eight_year_service_requirement(app):
    with app.app_context():
        agency, officer = make_telecommunicator(
        ...
        assert result["service_years"] == 6
        assert result["status"] == "NOT_ELIGIBLE"

        assert (
            "2 additional years of qualifying "
            "Telecommunicator service"
            in result["missing_requirements"]
        )


def test_500_training_hours_required(app):
    with app.app_context():
        agency, officer = make_telecommunicator()
        ...
        assert result["training_hours"] == 490
        assert result["status"] == "NOT_ELIGIBLE"
...
def test_equivalent_courses_are_accepted(app):
    with app.app_context():
        agency, officer = make_telecommunicator()
...
def test_missing_cultural_diversity_blocks_eligibility(app):
    with app.app_context():
        agency, officer = make_telecommunicator()
...
def test_all_requirements_establish_eligibility(app):
    with app.app_context():
        ...
        assert result["training_hours"] == 500
        assert result["missing_requirements"] == []


def test_master_certificate_reports_awarded(app):
    with app.app_context():
        agency, officer = make_telecommunicator()
